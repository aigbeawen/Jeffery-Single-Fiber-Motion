...
Q0=[0;-pi/2; 0.]; % Initial Orientation
...
fclose(fid); % fclose(vid); close(v1); close(v2);
%
% f4=figure(4); clf; hold on; fun_P=griddedInterpolant(tp,Plim,'spline','none'); 
% t2=linspace(0,max(tp),200); P2=fun_P(t2); plot(t2,P2(:,1),'b-',t2,P2(:,2),'r-');
% dat3d=load('.\Results\FEA_3D_2.txt','-ascii');[tf2,bf]=unique(dat3d(:,1),'sorted');
% fdat3d=griddedInterpolant(tf2,dat3d(bf,2:end),'makima','makima'); 
% dat3d2=fdat3d(t2); Pf2=dat3d2(:,end-1:end)*1.e3;
% plot(t2,Pf2(:,1),'b--',t2,Pf2(:,2),'r--');
% xlabel('\it Time [s]');ylabel('\it Pressure [kPa]'); 
% grid("on");set(gca,'GridLineStyle','--'); set(gca,'TickDir','Both');
% f4.Color='w'; set(gca,'FontSize',10); box("on");
% lg=legend('\it P_{min} - Jef','\it P_{max} - Jef');
% lg.NumColumns=2; lg.Location='southoutside';
% saveas(f4,sprintf('%s\\%s',dirname,'Pres_hist'),'png');
%
